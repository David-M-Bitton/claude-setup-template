...
description: Interactive step-by-step installer for the Google Workspace CLI (gws) on macOS, Linux, WSL, or native Windows (Git Bash/MINGW64). From a clean machine: detects platform, installs the package manager + Google Cloud SDK + gws CLI, helps accept GCP Terms of Service, creates a GCP project, enables chosen Workspace APIs, walks the user click-by-click through the Google Auth Platform consent screen + Desktop OAuth client (handling the multi-account-in-Chrome edge case), launches OAuth login, verifies the install with a real Gmail call, and runs a tailored first-use proof-of-concept against their actual workspace. Use whenever the user wants to install or set up the Google Workspace CLI / gws — phrases like "install gws", "set up Google Workspace CLI", "install gcloud for gws", "set up gws on my machine", "use Google Workspace from my terminal", or any variation of onboarding onto the gws CLI from scratch.
---
...
Realistic time: **5–10 minutes when `gws auth setup` completes on its own (the common case — try it first, Step 6), 15–20 minutes on the manual fallback if everything goes smoothly, 30–45 minutes for a first-time GCP user on the manual path** (Terms of Service + manual OAuth client creation add the extra time). Tell the user this upfront.
...
**Important nuance to flag honestly up front:** *Enabling* an API on the GCP project (Step 6b) is separate from *requesting an OAuth scope* for it (Step 7). 🔴 **Verified against the installed v0.22.5 binary:** with no scope flag at all, `gws auth login` requests a broad ~85-scope "recommended" set — and Google caps an unverified (testing-mode) OAuth app, which is what this guide creates, at **~25 scopes**. Left unflagged, login fails on most personal accounts. **Always pass `-s`/`--services` built from the Step 4 selection** (e.g. `-s drive,gmail,sheets,calendar,docs`) — see Step 7d. Tell the user this now so they aren't surprised later.
...
## Step 6: Configure GCP project, APIs, and OAuth

🔴 **Try the automated path first, every time — it's the documented fastest route once `gcloud` is authenticated (which Step 5 just did):**

```bash
gws auth setup --login
```

`gws auth setup --help` describes itself as: *"Configure GCP project + OAuth client (requires gcloud)."* Per the CLI's own README, this is the officially recommended path whenever `gcloud` is installed and authenticated. In one command it can create the GCP project, enable the Workspace APIs, create the Desktop OAuth client, and — with `--login` — chain straight into `gws auth login`.

Flags worth knowing:

| Flag | Does |
|---|---|
| `--project <id>` | Use a specific existing GCP project instead of creating one |
| `--login` | Run `gws auth login` automatically after setup succeeds |
| `--dry-run` | Preview every change with zero side effects — good for a first look |

**Branch on the result:**
- ✅ **It completes successfully** → if you passed `--login`, you're done — skip straight to **Step 8 (Verify)**. Otherwise run `gws auth login -s <services-from-step-4>` yourself (Step 7d explains why `-s` matters), then go to Step 8.
- ❌ **It errors** (it can still fail on some accounts or org policies) → show the user the exact error text, then fall through to the manual path below. Don't guess at a fix — the manual path handles every case.

*(Corrected: earlier versions of this guide assumed `gws auth setup` always fails in v0.22.5 and skipped straight to the manual Console walkthrough. Verified directly against the installed v0.22.5 binary that it works and is the documented fastest path — try it first.)*

---

### Manual fallback — only if the automated setup above errored, or the user wants explicit control

Take your time on this path. **This is where most installs fumble on first try — give exact instructions, not vague pointers.**
...
Continue to 6d — this is now the manual OAuth client walkthrough (the part `gws auth setup` normally does for you).
...
### 7d. Use `-s`/`--services` to stay under the 25-scope test-mode limit

🔴 **Verified against `gws auth login --help` on the installed v0.22.5 binary:** there are two different scope flags and they are NOT interchangeable.

| Flag | Expects | Use it? |
|---|---|---|
| `-s`, `--services <services>` | Short **service names** — `drive,gmail,sheets` | ✅ Yes — this is the one you want |
| `--scopes <scopes>` | Full OAuth **scope URIs** — `https://www.googleapis.com/auth/drive` | Only if hand-building a custom scope list |
| `--full` | Every scope incl. `pubsub` + `cloud-platform` | No — guaranteed to exceed the 25-scope test-mode cap |
| *(no flag)* | The ~85-scope "recommended" preset | No — same problem |

Google caps an unverified (testing-mode) OAuth app — which is what Step 6 just created — at **~25 scopes**. Both the no-flag default and `--full` blow past that on almost every personal account. **Build `-s` from the Step 4 selection every time:**

```bash
gws auth login -s drive,gmail,sheets,calendar,docs
```

*(Corrected: earlier versions of this guide said "don't pass `--scopes` shorthand" and pointed at the flagless default. Passing service names to `--scopes` does fail — but the fix is the `-s`/`--services` flag, not going flagless. Going flagless is the more common failure, since it silently requests ~85 scopes against a 25-scope cap.)*
...
| `403 accessNotConfigured` | API not enabled on the GCP project | `gws` prints a ready-to-click `enable_url` in its own error output — use that, or run `gcloud services enable <api>.googleapis.com --project <PROJECT_ID>` |
...
| `Some requested scopes were invalid` on auth login | You passed service names to `--scopes` (it wants full URIs). Use `-s`/`--services` instead: `gws auth login -s drive,gmail,sheets`. |
| Consent screen truncates scopes, or login fails with no clear error | Unverified test app is capped at ~25 scopes; flagless login and `--full` both request more (~85). Re-run with `-s <services>` (Step 7d). |
| `redirect_uri_mismatch` at OAuth | OAuth client wasn't created as **Desktop app** type. In Credentials, delete it, recreate as Desktop app, download the new JSON, redo Step 6h. |
| `403 accessNotConfigured` | Specific API not enabled on the project. `gws` prints a ready-to-click `enable_url` in its error output — use that, or `gcloud services enable <api>.googleapis.com`. |
...
| `gws auth setup` errors out | Not the common case in v0.22.5 — it usually succeeds (try it first, Step 6). If it genuinely fails, show the user the exact error, then walk through 6a–6h manually. |
...
- **Try `gws auth setup` first, every time (Step 6).** Only fall back to the manual Cloud Console walkthrough if it actually errors.
- **On the manual fallback, walk the user click-by-click through the Cloud Console UI.** Section 6a–6h is non-negotiable. Vague instructions ("set up the OAuth consent screen") fail. Every click, every field name, every dropdown choice. Use exact label text from the actual Google Auth Platform wizard, not paraphrases.
